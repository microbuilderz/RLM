...
static apr_filetype_e getType(const std::string & path)
{
	apr_finfo_t st;
	apr_status_t res = getInfo(path, APR_FINFO_TYPE, st);
	CHECK(res == APR_SUCCESS) << "Can't get info for '" << path << "', " << base::getErrorMessage();
...
std::string Path::getExtension(const std::string & path)
{
	...
	if (index == std::string::npos || index == 0 || index == path.size() -1)
		return "";
...
{
	char * pch;
	pch = strtok (const_cast<char*> (path.c_str()),Path::sep);
	while (pch != NULL)
	{
		std::string str(pch);
		parts.push_back(str);
		printf ("%s\n",pch);
		pch = strtok (NULL, Path::sep);
	}
}
...
{
	CHECK(!source.empty()) << "Can't copy from an empty source";

	CHECK(!destination.empty()) << "Can't copy to an empty destination";

	CHECK(source != destination)<< "Source and destination must be different";
...
{
	CHECK(!path.empty()) << "Can't remove an empty path";
...
{
	CHECK(!oldPath.empty() && !newPath.empty()) << "Can't rename to/from empty path";
#ifdef WIN32
