...
namespace Directory
{
	...
	std::string getCWD()
	{
		...
		CHECK(res > 0) << "Couldn't get current working directory. Error code: " << base::getErrorMessage();
#else
		cwd[0] = '\0';
	...
	void create(const std::string & path)
	{
		...
		if (Path::exists(p))
		{
			CHECK(Path::isDirectory(p)) << path << " already exists but it's not a directory";
			return;
		}
	...
	void Iterator::init(const std::string & path)
	{
		...
		CHECK(handle_) << "Can't open directory " << path << ". Error code: "    << errno;
#else
		...
		CHECK(res == 0) << "Can't open directory " << path << ". Error code: " << APR_TO_OS_ERROR(res);
#endif
	}
	...
	Iterator::~Iterator()
	{
		...
		CHECK(res == 0) << "Couldn't close directory." << " Error code: " << errno;
#else
		apr_status_t res = ::apr_dir_close(handle_);
		apr_pool_destroy(pool_);
		CHECK(res == 0) << "Couldn't close directory." << " Error code: " << APR_TO_OS_ERROR(res);
#endif
	}
	...
	void Iterator::reset()
	{
		...
		CHECK(res == 0) << "Couldn't reset directory iterator." << " Error code: " << APR_TO_OS_ERROR(res);
#endif
	}
	...
	Entry * Iterator::next(Entry & e)
	{
		...
		CHECK(!errno) << "Couldn't read next dir entry." << " Error code: " << errno;

		if (!p)
			return NULL;

		e.type = (p->d_type == DT_DIR) ? Directory::Entry::DIRECTORY : Directory::Entry::FILE;
		e.path = std::string(p->d_name);

		// Skip '.' and '..' directories
		if (e.type == Directory::Entry::DIRECTORY && (e.path == std::string(".") || e.path == std::string("..")))
			return next(e);
		else
			return &e;
	}
	...
	Entry * Iterator::next(Entry & e)
	{
		...
		if (res != 0)
		{
			CHECK(res == APR_INCOMPLETE) << "Couldn't read next dir entry." << " Error code: " << APR_TO_OS_ERROR(res);
			CHECK(((e.valid & wanted) | APR_FINFO_LINK) == wanted) << "Couldn't retrieve all fields. Valid mask=" << e.valid;
		}

		e.type = (e.filetype == APR_DIR) ? Directory::Entry::DIRECTORY : Directory::Entry::FILE;
		e.path = e.name;

		// Skip '.' and '..' directories
		if (e.type == Directory::Entry::DIRECTORY && (e.name == std::string(".") || e.name == std::string("..")))
			return next(e);
		else
			return &e;
	}
